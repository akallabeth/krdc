...
void RdpSession::channelConnected(void *context, const ChannelConnectedEventArgs *e)
{
    if (strcmp(e->name, CLIPRDR_SVC_CHANNEL_NAME) == 0) {
        ...
        // TODO: This is done like this:
        // 1. On local clipboard change, get all formats from clipboard, convert to RDP format and send to server
        // 2. On request from server generate the data and send. IMPORTANT: this must be done only on request or you´ll end up generating tons of data if you
        // copy something big like an image that can be announced in 10 foromats or similar
        // 3. There is the windows file clipboard that can copy & paste files. the common clipboard clipboard implements a wrapper for that, check xfreerdp for
        // details.
        // 4. If the server announces new formats place these in local QClipboard
        // 5. On paste (does QClipboard now allow async data generation?) request the data, block until received and then return
